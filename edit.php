...
// check for all required variables
    ...
    $editrepetition = optional_param('editrepetition',0, PARAM_INT );
...
//if are students suscribed to the event we can't edit the event except: name, descriptionn, place and visible    
    if( tutorias_students_suscribed_repetition_tutorship($id))
    {$editable_level=1;}
    else
    {$editable_level=0;}
...
//create and print the form
    ...
    if($editable_level==1)
    {
        ...
        if($editrepetition==1)
        {
            ...
            $result += tutorias_update_records($tabla, $tutorship, 'idrepetition');
            if($result)
            { 
        ...
        else
        {
            ...
            { 
                //Usamos un hack para que la direccion la muestre bien. le decimos que somos el bloque upload
                add_to_log($courseid,'upload','block_tutorias edited tutorship',$CFG->wwwroot.'/blocks/tutorias/view.php?eventid='.$tutorship->id.'&courseid='.$course->id.'&instanceid='.$instanceid,'tutorship edited');
                echo "<br>";
                    print_box(get_string('editedevent', 'block_tutorias'));
                if($block_tutorias->configdata->alowmulsendmail==1)
                {
                    $support_user=get_admin();
                    $subject=get_string('editsubject', 'block_tutorias',$tutorship->tutorshiptitle);
            ...
            {
                error(get_string('erroreditingevent', 'block_tutorias'));
                $error= mysql_error(); 
                //Usamos un hack para que la direccion la muestre bien. le decimos que somos el bloque upload
                add_to_log($courseid,'upload','block_tutorias error editing tutorship','','error in tutorship. Error:'.$error);
            }
        }
    ...
    else
    {
        ...
        {
            //if $editrepetition==1 the event is a repetition
            if($editrepetition==1)    {    
                $res=tutorias_delete_repetition_elements($id,$instanceid);
            }
            else
            {
                $res=delete_records('block_tutorias','id',$id);
            }
            if (!$res) 
            {
        ...
        else
        {
            ...
            {
                $res = tutorias_unsuscribe_students_repetition($id);
                $res = $res and (bool)delete_records('block_tutorias','id',$id);
            }
            if (!$res) 
            {
                error(get_string('deleterror','block_tutoriasl',$id));
    ...
    if($editrepetition==1)
    {
        print_box(get_string('editrepetition', 'block_tutorias'),"box noticebox noticeboxcontent boxaligncenter block_tutorias_warning_box");
    }
    ...
    if(($rep_time!=false)and ($editrepetition))
    {
        if($rep_time["day"]==14)
        {
            $toform['enablerepeat']=true;
            $toform['repeateach']='+1 fortnight';
            $toform['repetitionstart']=tutorias_get_date_start_repetition($instanceid,$idrep);
    ...
    $toform['idrepetition']=$event->idrepetition;
...
//We calculate how much time before is not abiable the torship
    ...
    $toform['freepositions']=$event->freepositions;
     ...
     $toform['editrepetition']=$editrepetition;
     $toform['eventid']=$id;
